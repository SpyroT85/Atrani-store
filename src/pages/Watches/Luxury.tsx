...
const Luxury = () => {
  return (
    <Layout>
      ...
      <div className="flex flex-col w-full gap-20 pb-24">
        {products['watches/luxury'].map((product, index) => (
            <MotionSection direction={index % 2 === 0 ? "left" : "right"} key={product.name}>
              <div className={`w-full flex justify-center${index === products['watches/luxury'].length - 1 ? ' pb-24' : ''}`}>
                <div
                  ...
                  {index % 2 === 1 ? (
                    <>
                      <div className={`flex flex-col items-start px-4 md:px-12 justify-self-start`} style={index === 0 ? { minWidth: '440px', marginLeft: '48px' } : undefined}>
                        ...
                        {(product.name === 'Luxury Brown' || product.name === 'Luxury Black' || product.name === 'Luxury Cream') && (
                          <>
                    ...
                    <>
                      ...
                      <div className={`flex flex-col items-start px-4 md:px-12`} style={index === 0 ? { minWidth: '440px', marginLeft: 'clamp(-260px, 18vw, -500px)' } : undefined}>
                        ...
                        {(product.name === 'Luxury Brown' || product.name === 'Luxury Black') && (
                          <>
                            <div style={{ height: '16px' }}></div>
                            <p className="text-black text-lg font-bold mb-2">Price: {product.price}€</p>
                            <div style={{ height: '16px' }}></div>
      ...
      <div className="w-full" style={{ paddingBottom: '6rem' }}></div>
    </Layout>
  );
};
