    ...
    <Layout>
      <section className="py-24 px-4 bg-white flex justify-center">
        <div className="max-w-5xl w-full">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-black text-center" style={{ marginBottom: '3rem' }}>Luxury</h1>
          <div className="flex flex-col w-full">
            {products.luxury.map((product, index) => (
              <div key={product.name} className={`flex ${index % 2 === 0 ? 'justify-start' : 'justify-end'} w-full`}>
                ...
                {index < products.luxury.length - 1 && <div className="h-24 md:h-32"></div>}
              </div>
            ))}
          </div>
